...
impl TestRunner {
    ...
    async fn poll_controller_for_compiled_ok(&self) -> bool {
        for _i in 0..40 {
            let resp = self.http_client
                .get(&format!("http://localhost/controller/get_status/{}", self.name))
                .body("")
                .send()
                .await
    ...
    async fn send_post_request(
        ...
        path_suffix: &str, 
        body: JsonValue,
        expected_mode: &str) -> String {
            let url = reqwest::Url::parse(&format!("http://localhost/dispatcher/{}/{}", self.name, path_suffix)).expect("parsing URL");
            let body = serde_json::to_vec(&body).expect("serializing JSON body");
            let resp = self.http_client.post(url).body(body).send().await.expect("sending request");
            let status = resp.status();
            let mode = resp.headers().get("X-Containerless-Mode").map(|mode| mode.to_str().unwrap().to_string());
            let resp_body = resp.text().await.unwrap();
            assert_eq!(status, 200, "got response code {} with body {}", status, resp_body);
...
pub async fn run_test(
    ...
    assert!(js_requests.len() > 0, "expected at least one pre-tracing request");
    assert!(rs_requests.len() > 0, "expected at least one post-tracing request");
    ...
    for (path_suffix, body) in js_requests.into_iter() {
        results.push(runner.send_post_request(&path_suffix, body, "tracing").await);
    }

    assert_eq!(runner.http_client.post(&format!("http://localhost/dispatcher/compile/{}", name))
        .body("").send().await.expect("sending compile request").status(), 200);
    ...
    for (path_suffix, body) in rs_requests.into_iter() {
        results.push(runner.send_post_request(&path_suffix, body, "decontainerized").await);
    }
