...
mod paths {
    ...
    lazy_static! {
        pub static ref GLOBAL_REGEX_SET: regex::RegexSet =
            regex::RegexSet::new(vec![r"^/log$"]).expect("Unable to create global regex set");
    }
    ...
    C: Has<XSpanIdString> + Send + Sync + 'static,
{
    ...
    fn call(&mut self, req: (Request<Body>, C)) -> Self::Future {
        async fn run<T, C>(
            mut api_impl: T, req: (Request<Body>, C),
...
impl<T> RequestParser<T> for ApiRequestParser {
    fn parse_operation_id(request: &Request<T>) -> Result<&'static str, ()> {
        ...
        match request.method() {
            // LogPost - POST /log
            &hyper::Method::POST if path.matched(paths::ID_LOG) => Ok("LogPost"),
            _ => Err(()),
        }
    }
}
