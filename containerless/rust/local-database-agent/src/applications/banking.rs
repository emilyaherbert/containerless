...
pub async fn commit_handler(transaction: AccountTransaction, accounts_db: DB<InternalAccount>) -> Result<impl warp::Reply, warp::Rejection> {
...
pub async fn balance_handler(account: AccountName, accounts_db: DB<InternalAccount>) -> Result<impl warp::Reply, warp::Rejection> {
    let accounts_db = accounts_db.lock().await;
    
    match accounts_db.get(&account.name) {
        Some(acc) => return response::ok_response(format!("{{ \"balance\": {} }}", (acc.balance.load(Ordering::SeqCst) / 100) as f64)),
        None => return response::ok_response("{ \"body\": \"Account not found.\" }".to_string())
    }
}