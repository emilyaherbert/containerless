...
use applications::{banking, login, status, Stack, DB};
use shared::logger;
...
async fn main() {
    ...
    users.insert(
        "emily".to_string(),
        login::User {
            username: "emily".to_string(),
            password: "herbert".to_string(),
        },
    );
    ...
    accounts.insert(
        "coffee".to_string(),
        banking::InternalAccount {
            name: "coffee".to_string(),
            balance: AtomicI64::new(10000),
        },
    );
    accounts.insert(
        "tea".to_string(),
